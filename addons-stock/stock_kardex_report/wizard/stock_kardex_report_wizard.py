...
class StockKardexReportWiz(models.TransientModel):
    ...
    date_from = fields.Datetime(string='From', required=True, default=lambda x:x.default_dates()['date_from'])
    date_to = fields.Datetime(string='To', required=True, default=lambda x:x.default_dates()['date_to'])
    product = fields.Many2one('product.product', required=True)
    location = fields.Many2one('stock.location', required=True, domain="[('usage', '=', 'internal')]")
